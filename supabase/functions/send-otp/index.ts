...
serve(async (req) => {
  ...
  try {
    ...
    if (existingSession?.otp_expires_at) {
      ...
      // Si el código actual todavía tiene más de 9 minutos de vida (envió hace < 1 min)
      if (diff > 540) {
    ...
    // 6. Envío vía Evolution API con Timeout
    ...
    return new Response(JSON.stringify({ error: err.message }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
      status: err.message.includes("Esperá") ? 429 : 500
    });
  }
});