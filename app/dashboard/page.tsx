...
export default function DashboardPage() {
  ...
  // ESTADOS DE NEGOCIO (SINCRONIZADOS CON DB)
  ...
  const t: any = {
    es: {
      ...
      kitDesc: 'Imprimí este QR para que tus clientes califiquen el local y reciban sus beneficios.',
      ...
      widgetTitle: 'Widget para tu Web', widgetDesc: 'Copiá este código para mostrar tus reseñas positivas en tu sitio.',
      reportTitle: 'Reporte Semanal WhatsApp', reportDesc: 'Recibirás un resumen de rendimiento todos los lunes.',
      staffPlaceholder: 'Nombres de tu equipo (separados por coma)...',
    ...
    pt: {
      ...
      kit: 'Kit de Marketing', downloadQr: 'Baixar QR Code',
      kitDesc: 'Imprima este QR Code para que seus clientes avaliem o local e recebam seus benefícios.',
      ...
      widgetTitle: 'Widget para seu Site', widgetDesc: 'Copie este código para exibir suas avaliações positivas no seu site.',
      reportTitle: 'Relatório Semanal WhatsApp', reportDesc: 'Você receberá um resumo de desempenho toda segunda-feira.',
  ...
  const applyBusinessData = (biz: any) => {
    setBizInfo(biz.brain_info || biz.business_info || '');
    ...
    <main className="min-h-screen bg-slate-950 text-white font-sans selection:bg-emerald-500/30">
      ...
      <div className="max-w-7xl mx-auto px-6 py-10">
        ...
        <div className="flex flex-wrap gap-4 mb-8">
          <button onClick={() => setViewMode('single')} className={`px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${viewMode === 'single' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/20' : 'bg-slate-900 text-slate-500 hover:text-white'}`}>{cur.singleView}</button>
          <button onClick={() => setViewMode('global')} className={`px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 ${viewMode === 'global' ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-500/20' : 'bg-slate-900 text-slate-500 hover:text-white'}`}><BarChart3 size={14}/> {cur.globalView}</button>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          {/* COLUMNA IZQUIERDA: INSIGHTS & STAFF */}
          <div className="lg:col-span-4 space-y-6">
            ...
            <div className="bg-slate-900/60 border border-white/5 p-6 rounded-[2.5rem]">
              ...
              <div className="space-y-3">
                {staffRanking.length === 0 ? <p className="text-[10px] uppercase text-slate-600 italic text-center py-2">Sin menciones aún</p> : staffRanking.map((s, idx) => (
                  <div key={idx} className="flex items-center justify-between bg-black/20 p-4 rounded-xl border border-white/5">
                    <span className="text-[11px] font-black italic uppercase tracking-tighter">{s.name}</span>
          ...
          {/* COLUMNA DERECHA: CONFIGURACIÓN PRINCIPAL */}
          <div className="lg:col-span-8 bg-slate-900 border border-white/5 rounded-[3rem] overflow-hidden flex flex-col shadow-2xl">
            <div className="p-2 flex bg-black/40 gap-2 border-b border-white/5">
              {[
                ...
                <button key={tab.id} onClick={() => setActiveTab(tab.id as any)} className={`flex-1 py-4 rounded-xl flex items-center justify-center gap-2 text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === tab.id ? 'bg-emerald-600 text-white shadow-inner' : 'text-slate-500 hover:text-white'}`}>
                ...
                <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  <div className="bg-emerald-500/5 p-6 rounded-2xl border border-emerald-500/10 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="text-left"><div className="text-[11px] font-black uppercase italic">{cur.reportTitle}</div><div className="text-[9px] text-slate-500 uppercase tracking-widest">{cur.reportDesc}</div></div>
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div className="space-y-3"><label className="text-[10px] font-black uppercase tracking-widest text-slate-500">{cur.lang}</label><select value={replyLang} onChange={(e) => setReplyLang(e.target.value)} className="w-full bg-slate-950 border border-white/5 p-4 rounded-xl text-xs font-bold outline-none"><option value="es">Español</option><option value="pt">Português</option></select></div>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-3"><label className="text-[10px] font-black uppercase tracking-widest text-slate-500">{cur.tone}</label><select value={replyTone} onChange={(e) => setReplyTone(e.target.value)} className="w-full bg-slate-950 border border-white/5 p-4 rounded-xl text-xs font-bold outline-none"><option value="friendly">{cur.toneFriendly}</option><option value="professional">{cur.toneProfessional}</option></select></div>
                  ...
                  <div className="space-y-3"><label className="text-[10px] font-black uppercase tracking-widest text-slate-500">{cur.brain}</label><textarea value={bizInfo} onChange={(e) => setBizInfo(e.target.value)} placeholder={cur.placeholderBrain} className="w-full bg-slate-950 border border-white/5 p-5 rounded-2xl text-xs min-h-[140px] outline-none focus:border-emerald-500 transition-all" /></div>
                </div>
              )}

              {activeTab === 'strategy' && (
                <div className="space-y-6 animate-in fade-in duration-500">
                  <div className="bg-slate-950/50 p-8 rounded-[2rem] space-y-8 border border-white/5 shadow-inner">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4"><div className="bg-emerald-500/10 p-3 rounded-xl"><CheckCircle className="text-emerald-500" size={24}/></div><div className="text-[12px] font-black uppercase italic tracking-tighter">{cur.auto5}</div></div>
              ...
              {activeTab === 'growth' && (
                <div className="space-y-8 animate-in fade-in duration-500">
                  <div className="bg-slate-950/50 p-8 rounded-[2rem] border border-white/5 space-y-8">
                    <div className="flex items-center justify-between"><div className="flex items-center gap-4"><div className="bg-indigo-500/10 p-3 rounded-xl"><Gift className="text-indigo-400" size={24}/></div><span className="text-[12px] font-black uppercase italic tracking-tighter">{cur.fidelization}</span></div><button onClick={() => setAutoCoupon(autoCoupon ? '' : 'Gift')} className={`w-14 h-8 rounded-full relative transition-all ${autoCoupon ? 'bg-indigo-600' : 'bg-slate-800'}`}><div className={`absolute top-1.5 w-5 h-5 bg-white rounded-full transition-all ${autoCoupon ? 'right-1.5' : 'left-1.5'}`} /></button></div>
                    <div className={`grid grid-cols-1 md:grid-cols-2 gap-8 items-center transition-all ${autoCoupon ? 'opacity-100' : 'opacity-30 grayscale pointer-events-none'}`}>
                      ...
                      <div className="bg-gradient-to-br from-slate-900 to-black border border-white/10 p-6 rounded-[2rem] shadow-2xl"><span className="text-[8px] font-black uppercase tracking-widest text-indigo-400 block mb-3">{cur.clientView}</span><h4 className="text-xl font-black italic tracking-tighter uppercase mb-1">{promoText || '...'}</h4><p className="text-[9px] text-slate-400 uppercase font-bold tracking-widest">Valid in {selectedBusiness?.business_name}</p></div>
                    </div>
                  </div>
                  
                  <div className="bg-indigo-500/5 p-8 rounded-[2rem] border border-indigo-500/10 space-y-6">
                    <div className="flex items-center gap-4"><div className="bg-indigo-400/20 p-3 rounded-xl"><Code className="text-indigo-400" size={24}/></div><div><div className="text-[11px] font-black uppercase italic">{cur.widgetTitle}</div><div className="text-[9px] text-slate-500 uppercase">{cur.widgetDesc}</div></div></div>
                    <div className="bg-black/60 p-5 rounded-2xl font-mono text-[10px] text-indigo-300 border border-white/5 relative group">
                      <div className="break-all pr-12">{`<iframe src="https://rankoai.com/widget/${selectedBusiness?.id}" width="100%" height="400" frameborder="0"></iframe>`}</div>
                      <button onClick={copyWidgetCode} className="absolute right-4 top-1/2 -translate-y-1/2 p-2 bg-white/10 rounded-lg hover:bg-white/20 transition-all">
                        {copied ? <Check size={16} className="text-emerald-400"/> : <Copy size={16}/>}
                      </button>
                    </div>
                  </div>

                  <div className="bg-slate-900/60 p-8 rounded-[2rem] border border-white/5 flex flex-col md:flex-row items-center gap-8 shadow-2xl">
                    <div className="bg-white p-4 rounded-2xl shadow-2xl shadow-white/5"><img src={`https://api.qrserver.com/v1/create-qr-code/?size=1000x1000&data=${encodeURIComponent("https://rankoai.com/review/" + selectedBusiness?.id)}`} alt="QR" className="w-32 h-32" /></div>
                    <div className="flex-1 space-y-4 text-center md:text-left">
                      <h4 className="text-sm font-black uppercase italic tracking-tighter">{cur.kit}</h4>
                      <p className="text-[10px] text-slate-500 uppercase tracking-widest leading-relaxed">{cur.kitDesc}</p>
                      <button className="bg-white text-black px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-3 mx-auto md:mx-0 hover:bg-emerald-400 transition-all"><Download size={14}/> {cur.downloadQr}</button>
                    </div>
                  </div>
                </div>
              )}
              ...
              {activeTab === 'staff' && (
                <div className="space-y-6 animate-in fade-in duration-500">
                  <div className="bg-slate-950/50 p-8 rounded-[2rem] border border-white/5 space-y-6 shadow-inner">
                    <div className="flex items-center gap-4"><div className="bg-indigo-500/10 p-3 rounded-xl"><Users className="text-indigo-400" size={24}/></div><span className="text-[12px] font-black uppercase italic tracking-tighter">{cur.staffTitle}</span></div>
                    <div className="space-y-4">
                      <label className="text-[10px] font-black uppercase tracking-widest text-slate-500">{cur.staffPlaceholder}</label>
                      <textarea value={staffNames} onChange={(e) => setStaffNames(e.target.value)} placeholder="Ej: Juan, Maria, Pedro..." className="w-full bg-slate-950 border border-white/5 p-6 rounded-2xl text-xs min-h-[120px] outline-none focus:border-indigo-500 transition-all" />
            ...
            <div className="p-6 bg-black border-t border-white/5 flex flex-col sm:flex-row items-center justify-between gap-6">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[9px] font-black uppercase tracking-widest text-slate-700 italic">RANKO ENGINE v4.1 // READY FOR BUILD</span>
              </div>
