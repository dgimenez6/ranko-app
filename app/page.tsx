...
export default function LandingPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      fetchData(user);
    } catch (e) { alert("Error al guardar cambios"); } finally { setIsSaving(false); }
  };

  if (loading) return (
    <div className="min-h-screen bg-black flex flex-col items-center justify-center gap-4">
      <Loader2 className="animate-spin text-emerald-500" size={40} />
      <span className="text-[10px] font-black tracking-[0.3em] text-slate-700 uppercase">Ranko AI Loading</span>
    </div>
  );
  ...
  if (!user) return (
    <div className="min-h-screen bg-[#020617] text-white flex flex-col font-sans selection:bg-emerald-500/30">
      <nav className="max-w-7xl mx-auto w-full px-6 py-8 flex justify-between items-center">
        <div className="flex items-center gap-3 min-w-fit pl-4">
      ...
      <div className="flex-1 flex flex-col items-center justify-center text-center px-6 max-w-5xl mx-auto space-y-12 pb-20">
        <h1 className="text-7xl md:text-9xl font-black italic tracking-tighter leading-[0.85] uppercase animate-in slide-in-from-bottom-8 duration-700">{t.heroTitle}</h1>
      ...
      <footer className="max-w-7xl mx-auto w-full px-6 py-16 border-t border-white/5">
        ...
        <div className="pt-16 text-center text-[9px] font-black text-slate-800 uppercase tracking-[0.5em]">© 2026 RANKO AI - SCALABLE HOSPITALITY SOLUTIONS</div>
      </footer>
    </div>
  );
      ...
      <nav className="border-b border-white/5 bg-black/50 backdrop-blur-md sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          <div className="flex items-center gap-4 min-w-0 pl-2">
            ...
            <span className="text-2xl font-black italic tracking-tighter uppercase bg-gradient-to-r from-indigo-500 via-cyan-400 to-emerald-400 bg-clip-text text-transparent whitespace-nowrap pr-4">Ranko AI</span>
          </div>
                ...
                <div className="space-y-8 animate-in fade-in duration-500">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div className="space-y-3">
                      ...
                      <input type="text" value={whatsappNumber} onChange={(e) => setWhatsappNumber(e.target.value)} className="w-full bg-slate-950 border border-white/5 p-3 rounded-xl text-xs font-bold outline-none focus:border-emerald-500 transition-all" />
                    </div>
                  </div>
                  <div className="space-y-3">
                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-500">{t.brainLabel}</label>
                    <textarea value={bizInfo} onChange={(e) => setBizInfo(e.target.value)} placeholder={t.brainPlaceholder} className="w-full bg-slate-950 border border-white/5 p-5 rounded-2xl text-xs min-h-[160px] outline-none focus:border-emerald-500 transition-all" />
                  </div>
                </div>
              )}
                  ...
                  <div className="bg-slate-950/50 p-6 rounded-2xl space-y-6 border border-white/5">
                    ...
                    {autoCoupon && (
                      <div className="space-y-3 pt-4 animate-in slide-in-from-top-2">
                        ...
                        <input type="text" value={promoText} onChange={(e) => setPromoText(e.target.value)} placeholder={t.couponPlaceholder} className="w-full bg-slate-900 border border-white/5 p-4 rounded-xl text-xs font-bold outline-none focus:border-emerald-500" />
                      </div>
                    )}
                  </div>
                </div>
