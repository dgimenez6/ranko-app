          ...
          <div className="fixed inset-0 bg-slate-950/95 backdrop-blur-md z-[100] flex items-center justify-center p-6 overflow-y-auto">
            <div className="bg-slate-900 border border-white/10 w-full max-w-3xl rounded-[3rem] p-8 md:p-12 shadow-2xl my-auto animate-in zoom-in-95">
              <div className="flex justify-between items-center mb-10">
                <h2 className="text-2xl font-black italic uppercase tracking-tighter italic tracking-tighter">{t.configTitle}: {selectedBusiness.business_name}</h2>
              ...
              <div className="grid md:grid-cols-2 gap-8">
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <select value={replyLang} onChange={(e) => setReplyLang(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-2xl p-5 text-xs font-bold outline-none appearance-none cursor-pointer uppercase italic tracking-tighter">
                ...
                <div className="space-y-6">
                  <div>
                    <label className="text-[10px] font-black uppercase text-slate-500 flex items-center gap-2 mb-3 italic tracking-tighter"><Sparkles size={14} className="text-indigo-400"/> {t.tone}</label>
                    <select value={aiTone} onChange={(e) => setAiTone(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-2xl p-5 text-xs font-bold outline-none appearance-none cursor-pointer uppercase italic tracking-tighter">
                  ...
                  <div>
                    <label className="text-[10px] font-black uppercase text-slate-500 flex items-center gap-2 mb-3 italic tracking-tighter"><Megaphone size={14} className="text-emerald-400"/> {t.promo}</label>
                    <input value={promoText} onChange={(e) => setPromoText(e.target.value)} type="text" placeholder="Ej: 10% OFF Martes..." className="w-full bg-white/5 border border-white/10 rounded-2xl p-5 text-xs font-bold outline-none focus:border-indigo-500 transition-all italic tracking-tighter" />
                  </div>
                  <div>
                    <label className="text-[10px] font-black uppercase text-slate-500 flex items-center gap-2 mb-3 italic tracking-tighter"><Phone size={14} className="text-emerald-400"/> {t.whatsapp}</label>
                    <input value={whatsappNumber} onChange={(e) => setWhatsappNumber(e.target.value)} type="text" placeholder="+54 9 11..." className="w-full bg-white/5 border border-white/10 rounded-2xl p-5 text-xs font-bold outline-none focus:border-indigo-500 transition-all italic tracking-tighter" />
                  </div>
                </div>
              </div>
